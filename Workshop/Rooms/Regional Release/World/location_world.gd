extends RefCounted
const Topology = preload("res://Workshop/Rooms/Regional Release/World/topology_generator.gd")
const Map = preload("res://Workshop/Rooms/Regional Release/World/hex_map.gd")
const Water = preload("res://Workshop/Rooms/Regional Release/World/local_water.gd")
const Terrain = preload("res://Workshop/Rooms/Regional Release/World/local_terrain.gd")
const Pois = preload("res://Workshop/Rooms/Regional Release/World/local_poi_placement.gd")
const Rivers = preload("res://Workshop/Rooms/Regional Release/World/river_generator.gd")
const Templates = preload("res://Workshop/Rooms/Regional Release/World/location_templates.gd")
const Contracts = preload("res://Workshop/Rooms/Regional Release/World/geographic_contracts.gd")
const State = preload("res://Workshop/Rooms/Regional Release/Persistence/map_state.gd")
const Storage = preload("res://Workshop/Rooms/Regional Release/Persistence/storage_paths.gd")
...
const Regional = preload("res://Workshop/Rooms/Regional Release/World/regional_state.gd")
var maps: Dictionary = {}
...
func declare(id: String, parent: String, template: String, label: String, constraints: Dictionary) -> void:
	assert(not records.has(id) and records.has(parent) and not Templates.get_template(template).is_empty(),"Workshop/Rooms/Regional Release/World/location_world.gd: invalid location declaration")
...
func ensure_location(id: String):
	...
	if not records.has(id):
		push_error("Workshop/Rooms/Regional Release/World/location_world.gd: unknown location " + id)
		return null
...
func request_poi(parent_id: String, template: String, requested_cell: Vector2i = Vector2i(-1,-1), label: String = "", hostility: int = -1) -> Dictionary:
	...
	var definition: Dictionary = Templates.get_template(template)
	if hostility < -1: return {"ok":false,"reason":"Workshop/Rooms/Regional Release/World/location_world.gd: invalid hostility strength."}
...
func read_state(id: String) -> Dictionary:
	...
	if not archives.has(id):
		push_error("Workshop/Rooms/Regional Release/World/location_world.gd: missing generated state " + id)
		return {}
...
func unload(id: String) -> bool:
	...
	if not previous_archive.is_empty() and DirAccess.remove_absolute(previous_archive) != OK:
		push_error("Workshop/Rooms/Regional Release/World/location_world.gd: obsolete map archive could not be removed: "+previous_archive)
	return true
...
func set_poi_hostility(id: String, strength: int) -> Dictionary:
	...
	if strength < 0 or local_id.is_empty() or id == local_id:
		return {"ok":false,"reason":"Workshop/Rooms/Regional Release/World/location_world.gd: invalid POI hostility request."}
