...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/Production Promotion/tests/world_test.gd: "+message)
func _initialize() -> void:
	...
	if saved.ok:
		...
		FileAccess.open("res://Workshop/Rooms/Production Promotion/tests/restart_path.txt",FileAccess.WRITE).store_string(saved.path)
...
func corruption_tests(sim) -> void:
	...
	for index: int in range(invalids.size()):
		var path: String = "res://Workshop/Rooms/Production Promotion/tests/saves/invalid_%d_%d.bin" % [Time.get_ticks_usec(),index]
