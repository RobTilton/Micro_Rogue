...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/Production Promotion/tests/actor_ui_test.gd: " + message)
func _initialize() -> void: call_deferred("run")
func run() -> void:
	var game = load("res://Workshop/Rooms/Production Promotion/tests/actor_fixture.tscn").instantiate()
	root.add_child(game)
	...
	print("Workshop/Rooms/Production Promotion/tests/actor_ui_test.gd: %d checks, %d failures" % [checks,failures])
