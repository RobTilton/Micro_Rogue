...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/Production Promotion/tests/autosave_test.gd: "+message)
func _initialize() -> void: call_deferred("run")
func run() -> void:
	var game = load("res://Production/main.tscn").instantiate()
	game.autosave_directory = "res://Workshop/Rooms/Production Promotion/tests/saves/autosave_validation/"
	...
	FileAccess.open("res://Workshop/Rooms/Production Promotion/tests/autosave_restart_path.txt",FileAccess.WRITE).store_string(path)
