...
func skill_available(actor: Dictionary, skill: String) -> bool:
	return actor.get("humanoid",true) and ready(actor) and skill in actor.skills and actor.clock.ready(skill) and next_weapon(actor).get("kind") == "sword"
...
func attack(actor: Dictionary, target_id: int) -> Dictionary:
	...
	var counter: bool = target.get("humanoid",true) and target.riposte
