extends SceneTree
const Sim = preload("res://Workshop/Rooms/Regional Foundation/Persistence/persistent_actor_world.gd")
...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/Regional Foundation/tests/autosave_test.gd: "+message)
func _initialize() -> void: call_deferred("run")
func run() -> void:
	create_timer(90.0).timeout.connect(func(): quit(2))
	var game = load("res://Workshop/Rooms/Regional Foundation/main.tscn").instantiate()
	game.autosave_directory = "res://Workshop/Rooms/Regional Foundation/tests/saves/autosave_validation/"
	game.snapshot_directory = game.autosave_directory.path_join("snapshots/")
	root.add_child(game)
	check(game.get_script().resource_path == "res://Workshop/Rooms/Regional Foundation/UI/world_game.gd","Room script identity")
	...
	FileAccess.open("res://Workshop/Rooms/Regional Foundation/tests/autosave_restart_path.txt",FileAccess.WRITE).store_string(path)
