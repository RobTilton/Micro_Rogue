...
func check(value: bool, label: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/Regional Foundation/tests/regional_test.gd: "+label)
...
func run() -> void:
	...
	check(local.links.has(Vector2i(20,20)),"Return starts at center")
	...
	for side: int in range(6):
		...
		for cell: Vector2i in boundary:
			check(local.contains(cell) and local.distance(local.spawn_cell,cell) == 20,"edge cell on radius")
			outline[cell] = true
	...
	var first: Dictionary = journal.checkpoint(world.snapshot(),"res://Workshop/Rooms/Regional Foundation/tests/saves/")
	...
	check(journal.checkpoint(world.snapshot(journal.previous),"res://Workshop/Rooms/Regional Foundation/tests/saves/").ok,"regional change delta saves")
