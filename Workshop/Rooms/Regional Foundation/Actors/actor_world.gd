extends RefCounted
const Maps = preload("res://Workshop/Rooms/Regional Foundation/World/map_world.gd")
const Actors = preload("res://Workshop/Rooms/Regional Foundation/Actors/actors.gd")
const Combat = preload("res://Workshop/Rooms/Regional Foundation/Actors/combat.gd")
const Inventory = preload("res://Workshop/Rooms/Regional Foundation/Actors/inventory.gd")
...
const Clock = preload("res://Workshop/Rooms/Regional Foundation/Actors/cooldowns.gd")
const Sight = preload("res://Workshop/Rooms/Regional Foundation/Actors/perception.gd")
var maps: RefCounted
...
func add_actor(actor: Dictionary, map_id: String, cell: Vector2i, faction: String, sprite: String = "goblin") -> int:
	assert(maps.maps.has(map_id) and maps.maps[map_id].walkable(cell) and actor_at(map_id,cell).is_empty(), "Workshop/Rooms/Regional Foundation/Actors/actor_world.gd: invalid or occupied actor spawn")
	var id: int = next_id
	next_id += 1
...
func travel(actor: Dictionary) -> Dictionary:
	...
	var arrival: Vector2i = arrival_cell(destination,link.get("arrival",Vector2i(1,3)))
	if arrival == Vector2i(-1,-1): return result(false,"The entrance is occupied; wait for space.")
