...
## Workshop-owned controller; Production is a read-only dependency.
...
const Encounters = preload("res://Workshop/Rooms/Living Adventurers/expedition_resolver.gd")
...
func retire_player() -> Dictionary:
	if not actors.has(player_id): return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: no player to retire.")
	...
	if actor.hp <= 0 or actor.get("retired",false) or maps.records[actor.map_id].template != "Town" or engaged(actor) or not actor.pending.is_empty() or actor.retreat:
		return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: retire alive, safely in town, after finishing pending actions.")
...
func load_game(path: String) -> Dictionary:
	if not path.simplify_path().begins_with("res://Workshop/Rooms/Living Adventurers/"):
		return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: only isolated Workshop saves can be loaded.")
	var file := FileAccess.open(path,FileAccess.READ)
	if file == null: return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: save not found.")
	var decoded: Variant
	...
	if not decoded is Dictionary: return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid save.")
	var reason: String = validate_workshop(decoded)
	if not reason.is_empty(): return result(false,reason)
	return super.load_game(path)

func save_game(directory: String = "res://Workshop/Rooms/Living Adventurers/Saves/snapshots/") -> Dictionary:
	if not directory.simplify_path().begins_with("res://Workshop/Rooms/Living Adventurers/"):
		return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: save path must stay in this Room.")
	var reason: String = validate_workshop(snapshot())
	if not reason.is_empty(): return result(false,reason)
	return super.save_game(directory)

static func validate_workshop(data: Dictionary) -> String:
	...
	if not state is Dictionary: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid ledger."
	...
	if state.get("version") != 1 or not state.get("steps") is int or state.steps < 0 or state.steps >= 6 or not state.get("turn") is int or state.turn < 0 or not state.get("retirement_order") is int or state.retirement_order < 0 or not state.get("seeded") is Array or not state.get("history") is Array:
		return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid scheduler state."
	if not state.get("far_cursor",0) is int or state.get("far_cursor",0) < 0: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid far cursor."
	if state.history.size() > 100: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: oversized history."
	for town_id: Variant in state.seeded:
		if not town_id is String or not data.records.has(town_id) or data.records[town_id].template != "Town": return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid seeded town."
	for event: Variant in state.history:
		if not event is Dictionary or not event.get("turn") is int or not event.get("actor") is int or not event.get("name") is String or not event.get("kind") is String or not event.get("details") is Dictionary: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid history."
	...
	for actor: Dictionary in data.actors.values():
		...
		if not job is Dictionary or not job.get("home") is String or not data.records.has(job.home) or data.records[job.home].template != "Town" or job.get("phase") not in ["resupply","quest","outbound","explore","return","claim","inn","rest","obscurity"] or not job.get("goal") is String or (not job.goal.is_empty() and not data.records.has(job.goal)):
			return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid adventurer goal."
		for field: String in ["born","retired_at","visits","rest_until"]:
			if not job.get(field) is int: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid adventurer counters."
		if not actor.get("retired",false) is bool: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid retirement flag."
		if job.has("journey") and (not job.journey is Dictionary or not job.journey.get("destination") is String or not job.journey.get("ready") is int): return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: invalid journey."
		...
		if not location.is_empty() and location != "global":
			...
			if presence[location] > MAX_RESIDENTS: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: physical population exceeds cap."
		var region: String = data.records[job.home].parent
		counts[region] = counts.get(region,0)+1
		if counts[region] > MAX_RESIDENTS: return "Workshop/Rooms/Living Adventurers/adventurer_world.gd: population exceeds cap."
	return ""
...
func move(actor: Dictionary, destination: Vector2i, expected: Array = []) -> Dictionary:
	if actor.has("adventure") and actor.map_id == "global":
		...
		if not target_region.is_empty() and physically_present(target_region,actor.id) >= MAX_RESIDENTS:
			actor.adventure.blocked = actor.adventure.get("blocked",0)+1
			return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: destination adventurer population is full.")
		if not journey_ready(actor,"global:"+str(destination)): return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: journey takes six world hours.")
...
func cross_border(actor: Dictionary, side: int) -> Dictionary:
	if actor.has("adventure"):
		for option: Dictionary in border_options(actor):
			...
			if physically_present(region,actor.id) >= MAX_RESIDENTS: return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: destination adventurer population is full.")
		if not journey_ready(actor,actor.map_id+":"+str(side)): return result(false,"Workshop/Rooms/Living Adventurers/adventurer_world.gd: journey takes six world hours.")
