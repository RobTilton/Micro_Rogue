...
const Resolve = preload("res://Production/Actors/expedition_resolver.gd")
var checks: int = 0
var failures: int = 0
...
func run() -> void:
	...
	check(World.validate_adventurers(world.snapshot()).is_empty(),"expedition keeps valid world and unique item ownership")
