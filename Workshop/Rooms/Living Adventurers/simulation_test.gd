...
const Resolver = preload("res://Workshop/Rooms/Living Adventurers/expedition_resolver.gd")
var checks: int = 0
var failures: int = 0
...
func run() -> void:
	...
	check(World.validate_workshop(world.snapshot()).is_empty(),"scheduled state validates")
	var saved: Dictionary = world.save_game("res://Workshop/Rooms/Living Adventurers/Tests/saves/")
	...
	check(not loaded.load_game("user://world.world").ok,"Workshop refuses production save paths")
	...
	check(World.validate_workshop(world.snapshot()).is_empty(),"retirement snapshot remains valid")
