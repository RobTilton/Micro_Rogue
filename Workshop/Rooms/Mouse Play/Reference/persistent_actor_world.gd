...
static func validate_snapshot(data: Dictionary) -> String:
	...
	for record: Dictionary in data.records.values():
		...
		for target in record.constraints.get("quests",{}):
			...
			if not quest is Dictionary or not data.records.has(target) or quest.get("target") != target or not quest.get("title") is String or quest.get("status") not in ["available","accepted","rewarded"] or not quest.get("reward") is int or quest.reward < 0: return "invalid bounty"
...
func quest_action(actor: Dictionary, target_id: String) -> Dictionary:
	...
	if actors.has(boss_id) and actors[boss_id].hp <= 0:
		...
		maps.revision += 1
		return result(true,"Bounty complete. Received "+str(quest.reward)+" gold.")
...
func _scale_monsters() -> void:
	...
	for actor: Dictionary in actors.values():
		...
		if not actor.has("base_roll"):
			...
			actor.boss = false
		var level: int = maxi(1,actors[player_id].level+actor.level_offset)
		if level == actor.level: continue
		var missing_hp: int = actor.max_hp-actor.hp
		actor.points += maxi(0,level-actor.level)
		actor.level = level
		actor.required_xp = 5*level
		for index: int in range(Actors.STATS.size()): actor.stats[Actors.STATS[index]] = roundi((actor.base_roll[index]+level-1)*(1.25 if actor.boss else 1.0))
