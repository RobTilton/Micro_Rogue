...
func check(condition: bool, description: String) -> void:
	...
	if not condition:
		failures += 1
		push_error("res://Workshop/Rooms/UI Foundation/Prototype/tests/tile_inspection_test.gd: " + description)
...
func run() -> void:
	...
	game.queue_free()
	await process_frame
	print("res://Workshop/Rooms/UI Foundation/Prototype/tests/tile_inspection_test.gd: %d checks, %d failures" % [checks,failures])
