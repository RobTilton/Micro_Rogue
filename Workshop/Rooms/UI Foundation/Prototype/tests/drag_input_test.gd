...
func verify(condition: bool, message: String) -> void:
	if not condition:
		failures += 1
		push_error("res://Workshop/Rooms/UI Foundation/Prototype/tests/drag_input_test.gd: " + message)
...
func run() -> void:
	...
	game = load("res://Workshop/Rooms/UI Foundation/Prototype/ui/main.tscn").instantiate()
	root.add_child(game)
	...
	game.queue_free()
	await process_frame
	print("res://Workshop/Rooms/UI Foundation/Prototype/tests/drag_input_test.gd: 8 input checks, %d failures" % failures)
