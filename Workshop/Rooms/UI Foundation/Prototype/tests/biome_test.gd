...
func _initialize() -> void:
	...
	assert(first.maps.global.biomes.size() == 3360,"biome_test.gd: coverage")
	...
	for cell: Vector2i in first.maps.global.biomes:
		...
		if first.maps.global.links.has(cell) and first.maps.size() < 20:
			assert(first.resolve(first.maps.global.links[cell]).region_biome == biome,"biome_test.gd: local metadata")
	print("biome_test.gd: biome coverage and sampled local metadata passed; seed 1729 distribution: ",counts)
	quit()
