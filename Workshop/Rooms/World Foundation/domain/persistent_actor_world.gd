extends "res://Workshop/Rooms/Actor Foundation/domain/actor_world.gd"
const Locations = preload("res://Workshop/Rooms/World Foundation/domain/location_world.gd")
const Templates = preload("res://Workshop/Rooms/World Foundation/domain/location_templates.gd")
...
const MapState = preload("res://Workshop/Rooms/World Foundation/domain/map_state.gd")
const Journal = preload("res://Workshop/Rooms/World Foundation/domain/autosave_journal.gd")
const SAVE_SCHEMA: int = 1
const SAVE_DIR: String = "res://Workshop/Rooms/World Foundation/saves/"
...
func snapshot(previous: Dictionary = {}) -> Dictionary:
	...
	if unchanged_geography:
		...
		for id: String in maps.maps:
			...
			if states[id].view_offset != map.view_offset or states[id].view_initialized != map.view_initialized:
				states = states.duplicate()
				states[id] = MapState.capture(map)
	return {"creation":creation.duplicate(true),"schema":SAVE_SCHEMA,"generator":Locations.GENERATOR_VERSION,"world_id":maps.world_id,"world_seed":maps.world_seed,"location_revision":maps.revision,"records":previous.records if unchanged_geography else maps.records.duplicate(true),"states":states,"next_instance":maps.next_instance,"actors":saved_actors,"ground":ground.duplicate(true),"initialized":initialized.duplicate(),"player_id":player_id,"next_id":next_id,"next_item_id":Items.next_item_id,"difficulty":difficulty,"tick":tick}
...
static func validate_snapshot(data: Dictionary) -> String:
	for key: String in ["schema","generator","world_id","world_seed","records","states","next_instance","actors","ground","initialized","player_id","next_id","next_item_id","difficulty","tick"]:
		if not data.has(key): return "missing "+key
	if data.schema != SAVE_SCHEMA or data.generator not in [1,Locations.GENERATOR_VERSION]: return "unsupported save/generator version"
	for key: String in ["records","states","actors","ground","initialized"]:
		if not data[key] is Dictionary: return "invalid "+key
	for key: String in ["world_seed","next_instance","player_id","next_id","next_item_id","difficulty","tick"]:
	...
	for id in data.actors:
		...
		if actor.pos.x < 0 or actor.pos.y < 0 or actor.pos.x >= state.dimensions.x or actor.pos.y >= state.dimensions.y or actor.pos in state.walls: return "invalid actor position"
