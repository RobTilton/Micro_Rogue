...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/World Foundation/tests/autosave_test.gd: "+message)
func _initialize() -> void: call_deferred("run")
func run() -> void:
	var game = load("res://Workshop/Rooms/World Foundation/main.tscn").instantiate()
	game.autosave_directory = "res://Workshop/Rooms/World Foundation/tests/saves/autosave_validation/"
	root.add_child(game)
	...
	FileAccess.open("res://Workshop/Rooms/World Foundation/tests/autosave_restart_path.txt",FileAccess.WRITE).store_string(path)
