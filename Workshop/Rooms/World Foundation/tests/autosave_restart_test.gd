...
func check(value: bool, message: String) -> void:
	...
	if not value:
		failures += 1
		push_error("Workshop/Rooms/World Foundation/tests/autosave_restart_test.gd: "+message)
func _initialize() -> void:
	var path: String = FileAccess.get_file_as_string("res://Workshop/Rooms/World Foundation/tests/autosave_restart_path.txt")
	var world = Sim.new(1)
	...
	check(journal.checkpoint(snapshot,"res://Workshop/Rooms/World Foundation/tests/saves/autosave_validation/").ok,"resume starts a durable new journal")
	...
	for filename: String in DirAccess.get_files_at("res://Workshop/Rooms/World Foundation/tests/saves/"):
		...
		var candidate: String = "res://Workshop/Rooms/World Foundation/tests/saves/"+filename
