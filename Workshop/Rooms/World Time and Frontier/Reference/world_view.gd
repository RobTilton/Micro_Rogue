...
const Variation = preload("res://Production/UI/terrain_variation.gd")
const DRAG_THRESHOLD: float = 7.0
const WastelandArt = preload("res://Production/UI/wasteland_art.gd")
var wasteland_textures: Array[Texture2D] = []
const MarshArt = preload("res://Production/UI/marsh_art.gd")
...
const PoiArt = preload("res://Production/UI/poi_art.gd")
...
func _sync_shop_roofs() -> void:
	if map_data == null: return
	if shop_map_id != map_data.id:
	...
	for cell: Vector2i in map_data.shops:
		...
		if not shop_nodes.has(cell):
			var sprite: Sprite2D = ShopRoofs.make(shop.roof)
			add_child(sprite)
