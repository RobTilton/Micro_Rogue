...
static func stock(map, seed_value: int) -> void:
	var rng := RandomNumberGenerator.new()
	rng.seed = Contracts.seed_for(seed_value,"town-stock")
	var prosperity: int = rng.randi_range(1,6)
	...
	for shop: Dictionary in map.shops.values():
		...
		for index: int in range(maxi(5,prosperity*3)):
			...
			if drop.ok:
				var price: int = maxi(1,5*drop.item.base_rank+drop.item.material_tier-1+int(drop.item.quality.modifier))
