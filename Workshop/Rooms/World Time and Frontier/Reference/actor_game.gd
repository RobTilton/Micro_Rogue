...
func _refresh() -> void:
	...
	if is_instance_valid(minimap): minimap.update_map(active_map,player.pos,visible_actors)
	board.visible_props = []
...
func _activate_panel(parent: Node) -> void:
	...
	for npc: Dictionary in simulation.on_map(player.map_id):
		...
		for quest: Dictionary in simulation.town_quests(player).values():
			...
			Parts.button(parent,text+" · "+str(quest.reward)+" gold",_crier_quest.bind(quest.target),quest.status != "rewarded")
...
func _quests_panel(parent: Node) -> void:
	...
	for record: Dictionary in map_world.records.values():
		for quest: Dictionary in record.constraints.get("quests",{}).values():
			count += 1
			...
			_wrap(parent,quest.title,360)
			Parts.label(parent,record.label+" · "+status+" · "+str(quest.reward)+" gold",16)
