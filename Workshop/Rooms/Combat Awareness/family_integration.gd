...
func run() -> void:
	...
	var dir: String = "res://Workshop/Rooms/Combat Awareness/tests/saves/"
	...
	var local_id: String = world.maps.records[town.id].parent
	var dungeon_id: String = ""
	for child: String in world.maps.records[local_id].children:
		if world.maps.records[child].template == "Dungeon": dungeon_id = child; break
	if dungeon_id.is_empty():
		var declared: Dictionary = world.maps.request_poi(local_id,"Dungeon")
		check(declared.ok,"declare test dungeon")
		for child: String in world.maps.records[local_id].children:
			if world.maps.records[child].template == "Dungeon": dungeon_id = child; break
	var dungeon = world.ensure_map({"id":dungeon_id})
	var families: Dictionary = world.maps.records[dungeon_id].constraints.population_families
	var count: int = 0
	for actor: Dictionary in world.actors.values():
		if actor.map_id != dungeon_id or actor.faction != "enemy": continue
		count += 1
		check(actor.family in [families.primary,families.rival] and actor.faction_id == actor.family,"room actor follows persistent faction roster")
		if actor.get("boss",false): check(actor.family == families.primary,"boss belongs to resident family")
	check(count > 1 and not dungeon.room_layout.is_empty(),"multiple room enemies use shared interior roster")
	check(Sim.validate_snapshot(world.snapshot()).is_empty(),"faction roster and actors validate for saving")
	...
	if DisplayServer.get_name() != "headless":
		...
		root.get_texture().get_image().save_png("res://Workshop/Rooms/Combat Awareness/tests/sidebar.png")
	game.queue_free()
	await process_frame
	print("Family integration: %d checks, %d failures" % [checks,failures])
