...
func run() -> void:
	...
	var dir: String = "res://Workshop/Rooms/Interior Presentation/tests/saves/"
	...
	var saved_props: Dictionary = town.props.duplicate(true)
	for kind: String in ["rug","rubble","chest"]:
		town.props[player.pos] = {"kind":kind,"name":kind.capitalize(),"opened":false,"contents":[],"gold":0,"room_id":-1}
		game._refresh()
		var text: String = game.board.item_tooltips[player.pos]
		check(("search to reveal" in text) == (kind == "chest"),"only containers advertise search: "+kind)
	town.props = saved_props
	game._refresh()
	...
	if DisplayServer.get_name() != "headless":
		...
		root.get_texture().get_image().save_png("res://Workshop/Rooms/Interior Presentation/tests/sidebar.png")
	game.queue_free()
	await process_frame
	print("Interior Presentation: %d checks, %d failures" % [checks,failures])
