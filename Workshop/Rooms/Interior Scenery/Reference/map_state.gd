...
const FIELDS: Array[String] = ["hex_radius","regional_revision","regional_values","id","title","layer","dimensions","wrap_horizontal","continents","spawn_cell","walls","view_offset","view_initialized","rivers","elevations","river_routes","water_cells","biomes","region_biome","links","shops","cave_layout","room_layout"]
...
static func restore(state: Dictionary):
	...
	for field: String in FIELDS: map.set(field,state.get(field,{}) if field in ["shops","cave_layout","room_layout"] else state[field])
	return map
static func valid(state) -> bool:
	if not state is Dictionary: return false
	for field: String in FIELDS:
		if field not in ["shops","cave_layout","room_layout"] and not state.has(field): return false
