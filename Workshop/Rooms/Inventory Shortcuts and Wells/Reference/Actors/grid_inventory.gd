...
static func _compatible(actor: Dictionary, item: Dictionary, slot: String) -> bool:
	return Rules.compatible(actor,item,slot)
...
static func valid(actor: Dictionary, ground: Array) -> bool:
	if not actor.get("main",{}).is_empty() and Rules.hands(actor,actor.main) == 2 and not actor.off.is_empty(): return false
	var seen: Dictionary = {}
	var placed: Array = []
	for item: Dictionary in actor.bag:
...
static func transfer(actor: Dictionary, ground: Array, actions: Dictionary, battle: bool, source: Dictionary, target: Dictionary, preview: bool = false, map = null) -> Dictionary:
	...
	match target.get("zone", ""):
		...
		"equipment":
			...
			if slot == "main" and item.get("hands","one") == "two" and not planned.off.is_empty():
				if not place_auto(planned.bag,planned.off): return {"ok":false,"reason":"Production/Actors/grid_inventory.gd: no backpack room for offhand equipment."}
				planned.off = {}
			if _compatible(planned,item,slot):
				...
				if accepted:
					planned[slot] = item
					if slot == "main": planned.grip = "two" if item.get("hands","one") == "two" else "one"
