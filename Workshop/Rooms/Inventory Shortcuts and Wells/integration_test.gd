...
func run() -> void:
	...
	var dir: String = "res://Workshop/Rooms/Inventory Shortcuts and Wells/tests/saves/"
	...
	var original_bag: Array = player.bag.duplicate(true)
	var belt: Dictionary = world.Items.make("belt")
	belt.contents.clear()
	for pouch: int in range(belt.capacity):
		var potion: Dictionary = world.Items.potion()
		potion.pouch = pouch
		belt.contents.append(potion)
	var belt_id: int = belt.item_id
	var potion_count: int = belt.contents.size()
	world.Grid.place_auto(player.bag,belt)
	game._quick_equip({"zone":"bag","id":belt_id})
	check(player.belt.get("item_id") == belt_id,"quick equip belt")
	check(world.transfer(player,{"zone":"equipment","slot":"belt","id":belt_id},{"zone":"empty_belt"}).ok,"empty equipped belt")
	check(player.belt.contents.is_empty() and player.bag.size() == potion_count,"all potions moved to backpack")
	var carried: Array = player.bag.duplicate(true)
	while world.Grid.place_auto(player.bag,world.Items.potion()): pass
	var extra: Dictionary = world.Items.potion()
	extra.pouch = 0
	player.belt.contents.append(extra)
	var before: Dictionary = player.duplicate(true)
	check(not world.Grid.transfer(player,world.ground[player.map_id],player.actions,false,{"zone":"equipment","slot":"belt","id":belt_id},{"zone":"empty_belt"}).ok and player == before,"full backpack refuses atomically")
	player.bag = carried
	var combat_actions: Dictionary = player.actions.duplicate(true)
	combat_actions.activation = 0
	combat_actions.free = 0
	combat_actions.exploration = false
	before = player.duplicate(true)
	check(not world.Grid.transfer(player,world.ground[player.map_id],combat_actions,true,{"zone":"equipment","slot":"belt","id":belt_id},{"zone":"empty_belt"}).ok and player == before,"combat requires activation without losing potions")
	combat_actions.activation = 1
	check(world.Grid.transfer(player,world.ground[player.map_id],combat_actions,true,{"zone":"equipment","slot":"belt","id":belt_id},{"zone":"empty_belt"}).ok and combat_actions.activation == 0,"combat empties belt for one activation")
	game._quick_equip({"zone":"equipment","slot":"belt","id":belt_id})
	check(player.belt.is_empty(),"quick unequip belt")
	player.bag = original_bag
	var well = world.ensure_map({"id":town.id+"/well"})
	check(well.dimensions == Vector2i(17,15) and well.walls.size() > 80,"well has a compact irregular chamber")
	check((well.links.size() == 2) == world.maps.well_has_underground(world.maps.records[well.id].seed),"well entrance follows 40 percent seeded roll")
	check(Sim.validate_snapshot(world.snapshot()).is_empty(),"optional well saves correctly")
	...
	if DisplayServer.get_name() != "headless":
		...
		root.get_texture().get_image().save_png("res://Workshop/Rooms/Inventory Shortcuts and Wells/tests/sidebar.png")
	game.queue_free()
	await process_frame
	print("Shortcuts and wells: %d checks, %d failures" % [checks,failures])
