...
func run() -> void:
	...
	var dir: String = "res://Workshop/Rooms/Enemy Roster/tests/saves/"
	game.autosave_directory = dir+"autosaves/"
	game.snapshot_directory = dir+"snapshots/"
	game.preferences_path = dir+"preferences.cfg"
	root.add_child(game)
	game._start_options()
	check(has_text(game.root,"Controls"),"start options exposes Controls")
	game._start_controls()
	check(has_text(game.root,"Finding the Global map") and has_text(game.root,"Back to Options"),"start controls and return navigation")
	game._start_options()
	...
	game._toggle_panel("Options",160)
	check(has_text(game.host.content,"Controls"),"game options exposes Controls")
	game._toggle_panel("Controls",160)
	check(has_text(game.host.content,"Finding the Global map") and has_text(game.host.content,"Inventory and belts"),"game controls builds shared reference")
	if DisplayServer.get_name() != "headless":
		await process_frame
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://Workshop/Rooms/Enemy Roster/controls.png")
	game._toggle_panel("Options",160)
	check(game.host.panel_name == "Options","return to game Options")
	game.host.close()
	...
	world._balance_chest_gold(town)
	check(town.props[chest_cell].gold == 2,"old chest gold quartered once, rounded down")
	...
	check(world.maps.maps[town.id].props[chest_cell].gold == 2,"world changes survive character death")
	check(game.minimap.get_parent() == game.rail and game.rail.get_child(0) == game.minimap,"single minimap is above sidebar")
	check(not world.discovered_maps.is_empty(),"UI records discovered maps")
	...
	if DisplayServer.get_name() != "headless":
		await process_frame
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://Workshop/Rooms/Enemy Roster/tests/sidebar.png")
	var gold_map = preload("res://Production/World/hex_map.gd").new(town.id,"Gold fixture","POI",Vector2i(10,10))
	gold_map.regional_values = {"hostility":4}
	for index: int in range(5):
		var kind: String = ["chest","crate","barrel","bones","corpse"][index]
		gold_map.props[Vector2i(index,1)] = {"kind":kind,"name":kind,"opened":false,"contents":[],"room_id":-1}
	world._seed_prop_gold(gold_map)
	check(gold_map.props[Vector2i(0,1)].gold in [3,6,9],"new chest gold follows quartered original formula")
	for index: int in range(1,5): check(gold_map.props[Vector2i(index,1)].gold == 0,"scenery containers and preplaced remains create no gold")
	var rng := RandomNumberGenerator.new()
	rng.seed = 87
	var monster: Dictionary = world._monster([3,3,3,3,3,3],rng,gold_map,false)
	check(monster.gold >= 2 and monster.gold <= 6,"new monster coin purse uses halved death gold")
	var legacy: Dictionary = world.Actors.create([3,3,3,3,3,3],true,rng)
	legacy.family = "wolves"
	legacy.can_use_skills = false
	legacy.skills = ["Lunge"]
	var spawn: Vector2i = world.arrival_cell(town,town.spawn_cell)
	var legacy_id: int = world.add_actor(legacy,town.id,spawn,"enemy")
	var possessions: Array = world.Inventory.possessions(legacy)
	var old_ids: Array = []
	for item: Dictionary in possessions: old_ids.append(item.item_id)
	check(not old_ids.is_empty(),"legacy wolf fixture owns equipment")
	var legacy_saved: Dictionary = world.save_game(dir+"legacy/")
	var migrated = Sim.new(1)
	check(legacy_saved.ok and migrated.load_game(world.last_save).ok,"legacy wolf save migrates")
	var creature: Dictionary = migrated.actors[legacy_id]
	check(not creature.humanoid and creature.enemy_variant == "wolves_1" and creature.main.is_empty() and creature.belt.is_empty(),"legacy wolf receives anatomy and natural attack")
	for id: int in old_ids:
		var retained: bool = false
		for drop: Dictionary in migrated.ground[town.id]:
			if drop.item.item_id == id: retained = true
		check(retained,"legacy possessions retained on ground")
	check(not migrated.skill_available(creature,"Lunge"),"legacy learned skill is unusable by creature")
	var upgraded_save: Dictionary = migrated.save_game(dir+"upgraded/")
	var roundtrip = Sim.new(1)
	check(upgraded_save.ok and roundtrip.load_game(migrated.last_save).ok and migrated.snapshot() == roundtrip.snapshot(),"upgraded roster world roundtrips without repeated migration")
	game.queue_free()
	await process_frame
...
func has_text(node: Node, wanted: String) -> bool:
	if (node is Label or node is Button) and node.text == wanted: return true
