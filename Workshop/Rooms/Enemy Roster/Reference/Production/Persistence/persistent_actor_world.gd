...
func _populate_rooms(map, layout: Dictionary, owner_key: String) -> void:
	var residents: Dictionary = room_families(maps.records[map.id].seed)
	...
	for room: Dictionary in layout.rooms:
		...
		if boss or rng.randf() < 0.6:
			...
			actor.family = residents.primary if boss or residents.rival.is_empty() or family_rng.randf() < 0.75 else residents.rival
			actor.faction_id = actor.family
...
static func room_families(seed_value: int) -> Dictionary:
	var rng := RandomNumberGenerator.new()
	rng.seed = Contracts.seed_for(seed_value,"resident-families")
	var families: Array = ["goblins","wolves","raiders"]
	var primary: String = families.pop_at(rng.randi_range(0,2))
	return {"primary":primary,"rival":families[rng.randi_range(0,1)] if rng.randf() < 0.5 else ""}
