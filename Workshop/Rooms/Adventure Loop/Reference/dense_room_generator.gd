...
static func generate(id: String, title: String, seed_value: int, tower: bool = false):
	...
	for row: int in range(rows):
		for column: int in range(columns):
			...
			rooms.append({"id":room_id,"center":Vector2i((xs[column]+xs[column+1]-2)/2,(ys[row]+ys[row+1]-2)/2),"cells":cells,"seed":Contracts.seed_for(seed_value,"room:"+str(room_id))})
			if column > 0:
...
static func valid(data, dimensions: Vector2i, walls: Array) -> bool:
	...
	if data.get("version") != 1 or data.get("entry_room") != 0 or not data.get("rooms") is Array or not data.get("doors") is Array: return false
	if data.rooms.size() < 9 or data.rooms.size() > 20: return false
	...
	for door in data.doors:
		...
		for direction: Vector2i in Map.DIRECTIONS:
			if owners.has(door.cell+direction): touches.append(owners[door.cell+direction])
		if a not in touches or b not in touches: return false
		adjacent[a].append(b)
		adjacent[b].append(a)
