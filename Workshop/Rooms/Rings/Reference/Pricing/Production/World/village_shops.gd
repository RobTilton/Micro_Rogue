...
static func retail_price(item: Dictionary) -> int:
	...
	if item.get("kind") == "ring": return 50 if item.greater else 10
...
static func sellable(item: Dictionary) -> bool:
	return item.get("kind","") in ["weapon","sword","shield","armor","belt","ring"] and (item.get("kind") != "belt" or item.get("contents",[]).is_empty())
