...
func walk_units(actor: Dictionary) -> int:
	return walks.get(actor.id,{}).get("remaining",(3+actor.stats.DEX)*2)
...
func walk_step(actor: Dictionary, destination: Vector2i) -> Dictionary:
	...
	var state: Dictionary = walks.get(actor.id,{"map_id":actor.map_id,"remaining":(3+actor.stats.DEX)*2,"charged":false})
