...
func check(condition: bool, message: String) -> void:
	...
	if not condition:
		failures += 1
		push_error("res://Production/Current/tests/floating_panels_test.gd: " + message)
...
func run() -> void:
	...
	game.queue_free()
	await process_frame
	print("res://Production/Current/tests/floating_panels_test.gd: %d checks, %d failures" % [checks,failures])
