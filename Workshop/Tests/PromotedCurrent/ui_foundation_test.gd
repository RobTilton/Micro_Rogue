extends SceneTree
const Game = preload("res://Production/Current/ui/workshop_game.gd")
...
func check(condition: bool, description: String) -> void:
	...
	if not condition:
		failures += 1
		push_error("res://Production/Current/tests/ui_foundation_test.gd: " + description)
...
func run() -> void:
	...
	game.queue_free()
	await process_frame
	print("res://Production/Current/tests/ui_foundation_test.gd: %d checks, %d failures" % [checks,failures])
