...
func check(condition: bool, description: String) -> void:
	checks += 1
	if not condition:
		push_error("res://Production/Current/tests/baseline_rules_test.gd: " + description)
		quit(1)
func _initialize() -> void:
	...
	print("res://Production/Current/tests/baseline_rules_test.gd: %d checks passed" % checks)
	quit()
