...
static func observe(world, actor: Dictionary) -> Dictionary:
	...
	for cell: Vector2i in visible:
		var entry: Dictionary = {"wall":cell in source.walls,"biome":source.biomes.get(cell,""),"water":source.water_cells.has(cell),"rivers":{},"loot":false}
		...
		memory[cell] = entry
	for drop: Dictionary in world.ground.get(source.id,[]):
		if visible.has(drop.pos): memory[drop.pos].loot = true
...
static func valid(value: Variant, states: Dictionary) -> bool:
	...
	for id in value:
		...
		if not states[id] is Dictionary or not states[id].get("dimensions") is Vector2i or not states[id].get("hex_radius") is int: return false
