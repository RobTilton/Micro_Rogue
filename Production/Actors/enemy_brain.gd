...
static func score(item: Dictionary) -> float:
	...
	if item.kind == "ring": return float(Rings.amount(item.family,item.greater))
...
func take_turn(world, actor: Dictionary) -> void:
	...
	if actor.retreat:
		...
		if not enemies.is_empty():
			for cell: Vector2i in world.paths(actor,ceili(Rings.stat(actor,"DEX")*0.5)):
	...
	if not target.is_empty():
		if map.distance(actor.pos,target.pos) > 1 and world.skill_available(actor,"Lunge"):
			for cell: Vector2i in world.paths(actor,Rings.stat(actor,"DEX")):
