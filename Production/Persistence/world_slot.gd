...
static func prune(keep: String, world_id: String, directories: Array[String], archive_directory: String = "") -> Dictionary:
	...
	if current.is_empty() or current.world_id != world_id: return {"ok":false,"reason":"Production/Persistence/world_slot.gd: replacement save is not complete."}
	...
	return {"ok":failures.is_empty(),"removed":obsolete.size()-failures.size(),"reason":"Production/Persistence/world_slot.gd: could not remove old saves: "+", ".join(failures) if not failures.is_empty() else ""}
