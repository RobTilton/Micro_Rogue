...
const COLLECTIONS: Array[String] = ["discovered_maps","records","states","actors","ground","initialized","regional_hexes","regional_sources"]
var path: String = ""
var previous: Dictionary = {}
var writes: int = 0
var compact_bytes: int = 32*1024*1024
...
func checkpoint(snapshot: Dictionary, directory: String = DIRECTORY) -> Dictionary:
	# Rotate to a full current-state checkpoint instead of growing history forever.
	# WorldSlot removes the predecessor only after this replacement is durable.
	if not path.is_empty():
		var existing := FileAccess.open(path,FileAccess.READ)
		if existing != null:
			var needs_compaction: bool = existing.get_length() >= compact_bytes
			existing.close()
			if needs_compaction: path = ""; previous = {}
