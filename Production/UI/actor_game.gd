...
func _shop_panel(parent: Node) -> void:
	...
	if shop.closed:
		...
		for entry: Dictionary in shop.get("stock",[]):
			if not preload("res://Production/World/village_shops.gd").buyable(entry.item): continue
			var buy_button: Button = Parts.button(parent,entry.item.name+" · "+str(preload("res://Production/World/village_shops.gd").stock_price(entry))+" gold",_buy_item.bind(entry.item.item_id),player.get("gold",0) >= preload("res://Production/World/village_shops.gd").stock_price(entry))
			var description: Dictionary = preload("res://Production/Actors/item_inspection.gd").describe(entry.item,true)
			buy_button.tooltip_text = description.title+"\n"+description.details+"\nPrice: "+str(preload("res://Production/World/village_shops.gd").stock_price(entry))+" gold"
...
func _request_trade(kind: String, item_id: int) -> void:
	...
	if kind == "buy":
		for entry: Dictionary in access.shop.stock:
			if entry.item.item_id == item_id: item = entry.item; price = preload("res://Production/World/village_shops.gd").stock_price(entry); break
	else:
	...
	if item.is_empty() or (kind == "buy" and not preload("res://Production/World/village_shops.gd").buyable(item)): return
...
func _confirm_trade() -> void:
	...
	if request.kind == "buy":
		for entry: Dictionary in access.shop.get("stock",[]):
			if entry.item.item_id == request.item_id: price = preload("res://Production/World/village_shops.gd").stock_price(entry); break
	else:
		for item: Dictionary in player.bag:
