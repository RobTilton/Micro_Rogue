...
func _draw() -> void:
	for q: int in range(7):
		for r: int in range(7):
			...
			for corner: int in range(6):
				points.append(center(cell) + Vector2.from_angle(deg_to_rad(60 * corner - 30)) * (SIZE - 2))
	...
	for loot_cell: Vector2i in loot_cells:
		if inside(loot_cell): draw_rect(Rect2(center(loot_cell) + Vector2(10, 8), Vector2(12,10)), Color("d99245"))
