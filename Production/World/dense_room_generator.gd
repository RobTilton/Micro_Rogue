...
static func generate(id: String, title: String, seed_value: int, tower: bool = false, small: bool = false):
	for attempt: int in range(48):
		var rng := RandomNumberGenerator.new()
		rng.seed = Contracts.seed_for(seed_value,"partition-"+str(attempt))
		var width: int = rng.randi_range(28,36) if small else (rng.randi_range(35,47) if tower else rng.randi_range(44,66))
		var height: int = rng.randi_range(26,34) if small else (rng.randi_range(35,47) if tower else rng.randi_range(40,60))
		var leaves: Array[Rect2i] = [Rect2i(1,1,width,height)]
		var target: int = rng.randi_range(6,9) if small else (rng.randi_range(9,13) if tower else rng.randi_range(12,20))
		...
		for trim: int in range(rng.randi_range(1,4)):
			if leaves.size() <= (6 if small else 8): break
	...
	if small:
		...
		for row: int in range(2):
			for column: int in range(3): rooms.append(Rect2i(1+column*10,1+row*10,8,8))
		var rng := RandomNumberGenerator.new()
		rng.seed = seed_value
		return _assemble(id,title,seed_value,rooms,Vector2i(30,20),rng)
